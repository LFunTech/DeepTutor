...
def test_protected_k8s_deploy_script_fails_fast_when_runtime_and_migrator_pg_dsn_match(
    ...
    same_dsn_b64 = "cG9zdGdyZXNxbDovL2R1bW15LXJ1bnRpbWUtbWlncmF0b3ItZHNu"
    fake_kubectl.write_text(
        f"""#!/usr/bin/env bash
set -euo pipefail
printf '%s\\n' "$*" >> "${{KUBECTL_LOG}}"
if [[ "$*" == *"get secret deeptutor-migrator-secrets"* ]]; then
  if [[ "$*" == *"DEEPTUTOR_POSTGRES_DATABASE_URL"* ]] || [[ "$*" == *"DEEPTUTOR_POSTGRES_MIGRATION_DATABASE_URL"* ]]; then
    printf '%s' "{same_dsn_b64}"
    exit 0
  fi
fi
if [[ "$*" == *"apply -f -"* ]]; then
