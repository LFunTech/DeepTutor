...
| DB migration | `DT_<ENV_KEY>_PG_MIGRATOR_DSN` 或 `DT_<ENV_KEY>_PG_MIGRATOR_SECRET_REF` | 迁移 Job/初始化步骤执行 schema migration、固定 tenant bootstrap | 仅 migrator 权限；不得使用普通 app runtime 超权账号；不得跨环境 DB | 推荐让 Job 从 SecretStore 拉取 DSN，Woodpecker 只持有 ref |
