# add-m1-fixed-tenant-runtime-baseline 执行证据

拆分说明：2026-09-24 用户确认将固定租户 runtime 与 Woodpecker/K8s 发布流水线拆成两个独立 proposal。本文件保留原 M1/G1 change 中与 runtime、smoke、resource binding、EduPlus2 demo 和边界保护相关的历史证据；A3/V.3 发布闭环证据迁移到 `add-g1-woodpecker-k8s-release-baseline` 后续补齐。
...
## 2026-09-24 proposal 拆分验证

| 命令 | 退出码 | 摘要 | 未验证项 / 下一步 |
| --- | --- | --- | --- |
| `openspec validate add-m1-fixed-tenant-runtime-baseline --strict` | 0 | Change valid。 | 当时 B2.3 固定租户伪造 tenant/resource/KB 负例仍按 tasks 保持未完成；后续见 2026-09-24 B2.3 补齐记录。归档需用户确认。 |
| `openspec validate --all --strict` | 0 | 16 items passed、0 failed。 | 仅验证 OpenSpec artifact；不替代目标 Woodpecker/K8s 发布。 |

## 2026-09-24 B2.3 固定租户多租户逃逸负例补齐

本轮只补齐 M1 固定租户 runtime 下的 B2 边界测试与一个 upstream-neutral 企业 policy 修正，不开放真实多租户/TMS，不新增 client/app registry 写接口，不操作目标集群。

新增/调整：

- `extensions/enterprise/src/deeptutor_enterprise/api/application.py`：企业生产 WebSocket `start_turn` policy hook 发现 body 中包含 `tenant`、`tenant_id` 或 `tenantId` 时 fail closed，避免“忽略伪造 tenant 字段即视为安全”的歧义。
- `extensions/enterprise/tests/test_application.py::test_m1_fixed_tenant_rejects_b2_escape_attempts`：覆盖伪造 tenant body、HTTP upload-intent body extra forbid、TMS client/app registry 写路径未开放、跨 tenant `resource_id` 不探测 ObjectStore、同租户私有 KB 不可见/required KB fail-closed、同租户私有 session 越权 404。
- 复用既有 `extensions/enterprise/tests/test_denials.py::test_client_tenant_override_and_http_denials_are_audited` 覆盖伪造 tenant header/query 与拒绝审计。

| 命令 / 验证 | 退出码 | 脱敏结果摘要 | 限制 / 后续 |
| --- | --- | --- | --- |
| `PYTHONPATH=extensions/enterprise/src:. ./.venv/bin/python -m pytest -c extensions/enterprise/pytest.ini -q extensions/enterprise/tests/test_application.py::test_m1_fixed_tenant_rejects_b2_escape_attempts`（RED） | 1 | 新增测试最初在 WS `start_turn` body `tenant_id` 处失败：`DID NOT RAISE ValueError`，证明企业生产 WS policy 会忽略伪造 tenant body。 | TDD RED，仅用于证明测试可捕获缺口。 |
| 同一测试（GREEN） | 0 | 1 passed；伪造 tenant body、跨 tenant resource、未开放 registry 写入、同租户私有 KB/session 越权均 fail closed。 | 本地隔离 PG/ASGI，不等同目标 K8s。 |
| `PYTHONPATH=extensions/enterprise/src:. ./.venv/bin/python -m pytest -c extensions/enterprise/pytest.ini -q extensions/enterprise/tests/test_application.py extensions/enterprise/tests/test_denials.py` | 0 | 29 passed、1 skipped；企业应用与入口拒绝回归通过。 | skip 为需显式开启的本地 MinIO 集成项，已有单独证据。 |
| `PYTHONPATH=. ./.venv/bin/python -m ruff check extensions/enterprise/src/deeptutor_enterprise/api/application.py extensions/enterprise/tests/test_application.py extensions/enterprise/tests/test_denials.py` | 0 | All checks passed。 | scoped lint。 |
| `openspec validate add-m1-fixed-tenant-runtime-baseline --strict && openspec validate --all --strict` | 0 | change valid；16 items passed、0 failed。 | OpenSpec artifact 校验，不替代目标 K8s/Woodpecker。 |
| targeted whitespace + secret scan against 本轮 4 个目标文件 | 0 | 未发现 trailing whitespace、私钥块、JWT 形态 token、长 `dt_token` 值或明文 secret/API key/password 赋值。 | 只扫描本轮目标文件；字段名/文档中的禁用词说明不视为泄露。 |

结论：B2.3 在当前仓库固定租户 runtime 下已补齐测试证据；M1 仍不开放 B2 多租户治理、TMS UI/API 或 client/app registry 写入口。
