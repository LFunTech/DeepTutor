...
- 默认 `pyproject.toml` 仍有 `aiosqlite` 且没有 PostgreSQL driver；首切片企业包已单独包含 PG driver，后续全量 change 才下沉到默认发行物。
- `deeptutor/services/session/__init__.py` 的 `get_session_store()` 已支持显式 provider（企业用 PG），未装配 provider 的默认路径仍选择 PocketBase/SQLite；用户已要求下一 change 取消该默认兼容。
...
## 当前 RAG provider 与服务接入基线

`services/rag/factory.py` 当前注册 `llamaindex`、`pageindex`、`pageindex-oss`、`graphrag`、`lightrag`、`lightrag-server`、`ima`、`weknora`，源码默认是 `llamaindex`，不是企业方案已采用某服务的声明。

WeKnora 的当前 client 使用 `base_url/api_key/knowledge_base_id` 调用 `/api/v1/knowledge-search`，返回片段并由 DeepTutor 回答；pipeline 同样拒绝本地导入且删除不操作远端内容。服务官方具备文档管理 API 不等于 DeepTutor adapter 已完成企业管理。该 adapter 保留为上游兼容基线，不纳入首发企业托管实现；对比证据见 [评估报告](evaluations/2026-09-13-rag-services.md)。

### LightRAG Server 现有支持
...
- 当前连接没有 per-request workspace 或最终用户授权契约；同一个 URL 下两个 KB 名称不意味着两个独立语料库。已选 LightRAG 的固定 workspace 企业拓扑见 [06](06-postgresql-native-store-plan.md)。

企业方案已确定用户 `LFunTech/LightRAG` fork，首发通过 HugeGraphStorage 存图，由 DeepTutor 回答。该支持已在本地 fork 工作区注册，但尚不能由当前 HEAD 完整复现，固定生产 commit/digest 和新组合验收仍待完成；不将其归为本仓库已实现功能。A1/A2 仍需交付完整企业接入并验证真实质量、生命周期、存储、安全与容量；不能从 HTTP 连接可用推断可投产或将现有存储自动升级为 PG/S3。
...
- 现有 `web/app/(admin)/admin` 是源码基线；企业目标入口采用 `/tms`，M1 保留固定租户管理功能，B2 按多租户和权限微调，不重写每租户 UI。该路径引用不表示当前已有 `/tms`；平台 `/oms` 是后续新增入口，见 [11](11-api-and-entrypoints.md)。
- PathService 不再提供 SQLite 运行库路径；普通既有文件载荷按各域契约处理，企业持久化走 Store/S3、scratch 可丢弃，不增加 tenant-local 文件后端。取消 SQLite profile 不代表全量文件替换已完成。
...
7. 首切片已在企业包提供 PG 身份/会话；后续全量 change 将通用 PG/迁移/Store 下沉 core 并接入默认入口，企业特有 `GrantStore`/`SettingsStore` 治理与 `ObjectStore` 仍按 A1/A2 完成，不能用只新增企业 Store 替代全仓调用收敛。
8. 直接调用 `get_sqlite_session_store()` 的路径需要收敛到 `get_session_store()` 或新的 store provider。

## 实施粒度提示

基线风险映射：结构化读写进入 A1，文件/检索进入 A2，K8s 集成与最终上线验收进入 A3（部署准备从 A1 并行）；B1 验证一个真实 EduPlus2 租户的最终接入，B2 才开放多租户与租户管理页面。C1/C2 是统一运营能力的两个工作包；H 多执行者协调按发布目标触发，不绑定 B2。A2 按已选 LightRAG Server 接入最终企业路径，验证稳定 namespace、全状态外置、S3 原文/解析物/派生副本和端到端生命周期，完成生产验收；若确有本地 `lightrag` 存量，才评估文件索引/绝对路径 workspace 的只读导入或重建，不默认重写本地 pipeline。
