...
| [`add-b2-eduplus2-tenant-lifecycle-webhook`](../../openspec/changes/add-b2-eduplus2-tenant-lifecycle-webhook/proposal.md) | 2/11；2026-09-26 已获批准，签名 Webhook URL mock 接收与外部控制台 demo 已完成 | 隔离合成 HMAC/时效/大小/状态不变测试通过；test-cn Secret 单字段同步、Woodpecker #58 发布及新 Pod 1/1 Ready，EduPlus2 `智能体基座` 控制台 8/8 个 mock 事件投递均为 HTTP 204。正式 Webhook 保持禁用，真实事件仍返回 503；`subscription.*` 权威版本/绑定/对账合同、PG inbox 和全入口准入仍待实现；OMS 额度不影响租户开停。 |
