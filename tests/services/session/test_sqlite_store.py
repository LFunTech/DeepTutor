...
def test_sqlite_store_default_constructor_uses_explicit_legacy_fixture_owner(
    tmp_path: Path,
) -> None:
    store = SQLiteSessionStore()

    assert store.db_path == tmp_path / "owner" / "user" / "chat_history.db"
    assert store.db_path.exists()
    assert store.store_scope.backend == "sqlite-fixture"


def test_sqlite_store_explicit_path_remains_available_for_legacy_fixtures(tmp_path: Path) -> None:
    store = SQLiteSessionStore(tmp_path / "legacy-fixture.db")

    assert store.db_path == tmp_path / "legacy-fixture.db"
    assert store.db_path.exists()
    assert store.store_scope.backend == "sqlite-fixture"
